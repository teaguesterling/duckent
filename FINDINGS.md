...
- **F4 generated object names collided.** `('a_b','c')` and `('a','b_c')` both compiled to `proj_a_b_c`, so the second create replaced the first's macro and either drop took out both; `tree_sql_object_name(kind, sch, nm)` doubles each part's underscores before joining, making the encoding injective (`sql/00_types.sql`, used everywhere).
...
- MN21's manifest claims it fails `test/sql/31_match.test`; it does not (the R-only match test there uses WHERE clauses only, so the lost TYPE default never shows). Same class of false claim as F14's MN02, but outside this wave's rulings: either a match test on an S-empty tree that leans on `_type = 'node'`, or a narrower `expect_fail`.
