...
-- raises where the original returns the text 'false'. The copy is GENERATED verbatim from
-- the source macro by test/mutants/regen.py; .control.sql is the same copy with the edit left
-- out, so a diff between the two IS the mutation.
...
CREATE OR REPLACE MACRO tree_sql_clause(kind, value, op, arg, alias, attr_cols, has_map, p, elem) AS
  CASE kind
    ...
    -- the mutation
    WHEN 'pseudo_unknown' THEN tree_err('tree_match: unknown pseudo-class ' || value)
